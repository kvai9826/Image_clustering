...
# CONFIG
# ----------------------------
DB_FILE = "updated_claims_final.db"
IMAGE_FOLDER = "./images"
...
menu = st.sidebar.radio("Navigate", ["Submit Claim", "Database Viewer"])
...
# ----------------------------
    ...
    if df.empty:
        ...
        for uid in df["unique_image_id"].unique():
            ...
            total_cluster_value = cluster_df["value_usd"].dropna().sum()
            ...
            for sc in cluster_df["sub_cluster_id"].unique():
                ...
                total_value = sub["value_usd"].dropna().sum()
                account_count = len(sub)

                st.caption(f"Subcluster: {sc}  |  {account_count} Accounts  |  Total: ${total_value:,.2f}")
